...
export default function RegisterPage() {
  ...
  const handleSendOtp = (e: React.FormEvent) => {
    e.preventDefault();
    axios.post('http://localhost:3000/user/Sendotp', { email })
      .then((res) => {
        if (res.data.success) {
          setOtp(res.data.otp);
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    e.preventDefault();
    axios.post('http://localhost:3000/user/verifyotp', { email, otp })
      .then((res) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    axios.post('http://localhost:3000/user/reg', { name, email, password, address })
      .then((res) => {
        if (res.data.success) {
          navigate("/login");
        }
          ...
          {step === 'details' && (
            <>
              ...
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="bakery@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="address">Address</Label>
                <LoadScript googleMapsApiKey={VITE_GOOGLE_MAPS_API_KEY} libraries={["places"]}>
                  <Autocomplete
          ...
          {step === 'otp' && (
            <>
              <div className="space-y-2">
                ...
                <Input id="otp" type="text" placeholder="Enter the OTP sent to your email" value={otp} onChange={(e) => setOtp(e.target.value)} required />
              </div>
